...
namespace RazorLibrary.HttpServices
{
    public class LoginHttpService
    {
        ...
        public LoginHttpService(HttpClient httpClient, IConfiguration configuration)
        {
        ...
        #region User Login 
        public async Task<ResponseOutcome<UserAccount>> Login(UserLoginDto userLoginDto)
        {
            ResponseOutcome<UserAccount> outcome = new ResponseOutcome<UserAccount>();

            try
            {
                using (HttpResponseMessage response = await _httpClient.PostAsJsonAsync("UserAccount/login", userLoginDto))
                {
